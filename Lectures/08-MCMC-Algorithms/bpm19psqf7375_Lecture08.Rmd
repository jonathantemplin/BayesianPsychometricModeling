...
$$Y_r = \beta_0 + \beta_1 x_{r1} + \beta_2 x_{r2} + \dots + \beta_V x_{rV} + e_r = \boldsymbol{x}_r \boldsymbol{\beta} + e_r$$
...
The right-hand side is the vector notation of the regression equation for a single respondent, with $\boldsymbol{\beta} = \left[\beta_0, \beta_1, \dots, \beta_V \right]^T$ a vector of regression coefficients with size $(V+1) \times 1$ and $\boldsymbol{x}_r = \left[1, x_{r1}, \dots, x_{rV} \right]$ a vector of a constant of 1 (for multiplying the intercept $\beta_0$) and $V$ predictors, for a size of  $1 \times (V+1)$. 
...
- $\boldsymbol{e}$ is size $N \times 1$ and $\boldsymbol{e} \sim MVN \left(\boldsymbol{0}, \boldsymbol{\Sigma}_e = \sigma^2_e \boldsymbol{I}_{(N \times N)} \right)$, where:
...
Here, note the determinant of $\left| \Sigma_e \right| = \left| \sigma^2_e \boldsymbol{I} \right| = \left| \sigma^2_e \right| \left| \boldsymbol{I} \right| =\left( \sigma^2_e\right)^N = \sigma^{2N}_e$

$$f(\boldsymbol{Y}|\boldsymbol{X}, \boldsymbol{\beta}, \sigma^2_e) = \frac{1}{\sqrt{\left(2 \pi \right)^V \left| \sigma^2_e \boldsymbol{I} \right|} }
\exp \left( \frac{1}{2} \left( \boldsymbol{Y} - \boldsymbol{X}\boldsymbol{\beta} \right)^T \left( \sigma^2_e \boldsymbol{I} \right)^{-1} \left( \boldsymbol{Y} - \boldsymbol{X}\boldsymbol{\beta} \right) \right) = \left( 2\pi \right)^{-\frac{V}{2}} \left(\sigma^2\right)^{-\frac{N}{2}} \exp{\left(- \frac{1}{2\sigma^2_e} \left( \boldsymbol{Y} - \boldsymbol{X}\boldsymbol{\beta} \right)^T \left( \boldsymbol{Y} - \boldsymbol{X}\boldsymbol{\beta} \right) \right)} $$

Often, the normalizing constants are removed, which leaves:
$$f(\boldsymbol{Y}|\boldsymbol{X}, \boldsymbol{\beta}, \sigma^2_e) \propto \left(\sigma^2_e \right)^{-\frac{N}{2}} \exp{\left(- \frac{1}{2\sigma^2_e} \left( \boldsymbol{Y} - \boldsymbol{X}\boldsymbol{\beta} \right)^T \left( \boldsymbol{Y} - \boldsymbol{X}\boldsymbol{\beta} \right) \right)} $$


We will come back to this distribution later for now, please note that because $\boldsymbol{\Sigma}_e = \sigma^2_e \boldsymbol{I}_{(N \times N)}$, an equivalent expression is possible using a series of independent univariate normal distributions:

$$f(\boldsymbol{Y}|\boldsymbol{X}, \boldsymbol{\beta}, \sigma^2_e) = \prod_{r=1}^N \frac{1}{\sqrt{2\pi\sigma^2_e}} \exp{\left(\frac{-(Y_r - \boldsymbol{X}_r \boldsymbol{\beta})^2 }{2\sigma^2_e}\right)}$$

Alternatively, $\left(Y_r | \boldsymbol{X}_r, \boldsymbol{\beta}, \sigma^2_e \right) \sim N\left(\boldsymbol{X}_r \boldsymbol{\beta}, \sigma^2_e \right)$.
...
\left( \boldsymbol{Y} - \boldsymbol{X}\hat{\boldsymbol{\beta}} \right)^T \left( \boldsymbol{Y} - \boldsymbol{X}\hat{\boldsymbol{\beta}} \right) + 
\left(\boldsymbol{\beta} - \hat{\boldsymbol{\beta}}\right)^T\left(\boldsymbol{X}^T\boldsymbol{X} \right) \left(\boldsymbol{\beta} - \hat{\boldsymbol{\beta}} \right)
$$
...
$$ f(\boldsymbol{Y}|\boldsymbol{X}, \boldsymbol{\beta}, \sigma^2_e) \propto \left( \sigma^2_e\right)^{-\frac{\nu}{2}} \exp\left( - \frac{\nu \sigma^2_0}{2\sigma_e^2}\right)
      \left( \sigma^2_e\right)^{-\frac{N-\nu}{2}} \exp \left( - \frac{1}{2\sigma^2_e} \left(\boldsymbol{\beta} - \hat{\boldsymbol{\beta}}\right)^T\left(\boldsymbol{X}^T\boldsymbol{X} \right) \left(\boldsymbol{\beta} - \hat{\boldsymbol{\beta}} \right)\right)
$$
Where $\nu = N-V$.
...
The posterior distribution becomes a mess, but it can be summarized by looking first at $\sigma^2_e$ and the at $\boldsymbol{\beta}$. The entire derivation won't be provided, however, the joint posterior distribution $f\left(\boldsymbol{\beta}, \sigma^2_e \mid \boldsymbol{Y}, \boldsymbol{X} \right)$ is a product of inverse-gamma and normal distributions.
...
$$ f \left(\boldsymbol{z} \mid \boldsymbol{m, V} \right) \propto \exp\left(-\frac{1}{2} \left(\boldsymbol{z}^T \boldsymbol{V}^{-1}\boldsymbol{z} - 2 \boldsymbol{z}^T  \boldsymbol{V}^{-1} \boldsymbol{m}\right) \right)$$
...
$$f(\boldsymbol{Y}|\boldsymbol{X}, \boldsymbol{\beta}, \sigma^2_e) = \left(2 \pi \sigma^2_e \right)^{-\frac{N}{2}} \exp{\left(- \frac{1}{2\sigma^2_e} \left( \boldsymbol{Y} - \boldsymbol{X}\boldsymbol{\beta} \right)^T \left( \boldsymbol{Y} - \boldsymbol{X}\boldsymbol{\beta} \right) \right)} $$
...
$$ \frac{1}{\sigma^2_e}\left( \boldsymbol{Y} - \boldsymbol{X}\boldsymbol{\beta} \right)^T \left( \boldsymbol{Y} - \boldsymbol{X}\boldsymbol{\beta} \right) =
    \frac{\boldsymbol{Y}^T\boldsymbol{Y}}{\sigma^2_e} - \frac{2 \boldsymbol{\beta}^T\boldsymbol{X}^T\boldsymbol{Y}}{\sigma^2_e} + 
    \frac{\boldsymbol{\beta}^T\boldsymbol{X}^T \boldsymbol{X}\boldsymbol{\beta}}{\sigma^2_e}
$$
...
$$\left( \boldsymbol{\beta}^T \boldsymbol{\Sigma}_0^{-1} \boldsymbol{\beta} - 2\boldsymbol{\beta}^T\boldsymbol{\Sigma}_0^{-1} \boldsymbol{\beta}_0 +
   \boldsymbol{\beta}^T_0 \boldsymbol{\Sigma}_0^{-1} \boldsymbol{\beta}_0\right) + \left( \frac{\boldsymbol{Y}^T\boldsymbol{Y}}{\sigma^2_e} - \frac{2 \boldsymbol{\beta}^T\boldsymbol{X}^T\boldsymbol{Y}}{\sigma^2_e} + 
    \frac{\boldsymbol{\beta}^T\boldsymbol{X}^T \boldsymbol{X}\boldsymbol{\beta}}{\sigma^2_e} \right) $$
...
$$ \boldsymbol{\beta}^T \boldsymbol{\Sigma}_0^{-1} \boldsymbol{\beta} + 
    ...
    - \frac{2 \boldsymbol{\beta}^T\boldsymbol{X}^T\boldsymbol{Y}}{\sigma^2_e}   
    $$

Then, combining like terms for the first two parts and the second two parts leaves:

$$
\boldsymbol{\beta}^T \left( \boldsymbol{\Sigma}_0^{-1} + \frac{\boldsymbol{X}^T \boldsymbol{X}}{\sigma^2_e}  \right) \boldsymbol{\beta} 
 - 2\boldsymbol{\beta}^T \left( \boldsymbol{\Sigma}_0^{-1} \boldsymbol{\beta}_0  + \frac{\boldsymbol{X}^T\boldsymbol{Y}}{\sigma^2_e}   \right)
$$
...
$$\boldsymbol{\Sigma}_n^{-1} = \left( \boldsymbol{\Sigma}_0^{-1} + \frac{\boldsymbol{X}^T \boldsymbol{X}}{\sigma^2_e}  \right), $$

and variance inverse times mean: 

$$\boldsymbol{\Sigma}_n^{-1} \boldsymbol{\beta}_n = \left( \boldsymbol{\Sigma}_0^{-1} \boldsymbol{\beta}_0  + \frac{\boldsymbol{X}^T\boldsymbol{Y}}{\sigma^2_e}   \right) $$

Therefore the mean vector is:
$$\boldsymbol{\beta}_n = \boldsymbol{\Sigma}_n^{-1} \left( \boldsymbol{\Sigma}_0^{-1} \boldsymbol{\beta}_0  + \frac{\boldsymbol{X}^T\boldsymbol{Y}}{\sigma^2_e}   \right) =
 \left( \boldsymbol{\Sigma}_0^{-1} + \frac{\boldsymbol{X}^T \boldsymbol{X}}{\sigma^2_e}  \right)\left( \boldsymbol{\Sigma}_0^{-1} \boldsymbol{\beta}_0  + \frac{\boldsymbol{X}^T\boldsymbol{Y}}{\sigma^2_e}   \right)
$$
...
$$\bar{\boldsymbol{\beta}} = \left(\Sigma_0^{-1} + \boldsymbol{X}^T\boldsymbol{X} \right)^{-1}\left(\Sigma_0^{-1} \boldsymbol{\beta}_0  +  \boldsymbol{X}^T\boldsymbol{Y} \right)$$
...
$$f(\sigma^2_e | \boldsymbol{\beta}, \boldsymbol{X}, \boldsymbol{Y} ) \propto  \left[ \left( \sigma^2_e\right)^{-\frac{\nu_0}{2} +1} \exp\left( - \frac{\nu_0 \sigma^2_0}{2\sigma_e^2}\right) \right] \times \left[ \left( \sigma^2_e \right)^{-\frac{N}{2}} \exp{\left(- \frac{1}{2\sigma^2_e} \left( \boldsymbol{Y} - \boldsymbol{X}\boldsymbol{\beta} \right)^T \left( \boldsymbol{Y} - \boldsymbol{X}\boldsymbol{\beta} \right) \right)} \right]
$$


$$ = \left( \sigma^2_e\right)^{-\frac{\nu_0 - N}{2}} \exp\left( - \frac{\nu_0 \sigma^2_0 +  \left( \boldsymbol{Y} - \boldsymbol{X}\boldsymbol{\beta} \right)^T \left( \boldsymbol{Y} - \boldsymbol{X}\boldsymbol{\beta} \right)}{2\sigma_e^2} \right)
$$

This is the form of an inverse gamma distribution, so the posterior distribution for $\sigma^2_e$ is inverse gamma, with: 

$$\alpha_n = \frac{\nu_0+N}{2}$$
$$\beta_n = \frac{\nu_0 \sigma^2_{0} + \left(\boldsymbol{Y} - \boldsymbol{X}\boldsymbol{\beta} \right)^T\left(\boldsymbol{Y} - \boldsymbol{X}\boldsymbol{\beta} \right)}{2}$$
