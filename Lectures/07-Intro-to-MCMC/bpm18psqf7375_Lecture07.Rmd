...
needed_packages = c("invgamma", "ggplot2", "rjags", "R2jags", "HDInterval")
...
To demonstrate each type of algorithm, we will use a model for a normal distribution (Chapter 4 of Levy & Mislevy, 2016) 
...
gelman.diag(BayesianFullModel.Posterior) 
...
Reasons for differences: $\sigma^2_e$. Look at what happens if we fix the residual variance to that it was in the least squares analysis (now using the R2jags package to save some space)
...
BayesianFullModel.Results = jags(data = BayesianFullModel.JAGS.Data, inits = BayesianFullModel.Init.Values, parameters.to.save = BayesianFullModel.Parameters,
...
BayesianFullModel.Results2 = jags(data = BayesianFullModel.JAGS.Data, inits = BayesianFullModel.Init.Values, parameters.to.save = c(BayesianFullModel.Parameters, "gamma.0"),
...
$$ D = -2 \log \left( p \left(y \mid \boldsymbol{\theta} \right) \right) + C$$
